...
+pad(d.getUTCHours())+':'
+pad(d.getUTCMinutes())+':'
+pad(d.getUTCSeconds())+'Z';}},when:{localStorageAvailable:$.Deferred()}});if(CL.url.baseDomain){document.domain=CL.url.baseDomain;$('body').append('<iframe style="display: none;" src="//www.'+CL.url.baseDomain+'/about/localstorage"></iframe>');}else{CL.when.localStorageAvailable.reject();}
...
function issueFormatCookie(format){var date=new Date();date.setTime(date.getTime()+(365*24*60*60*1000));document.cookie='cl_fmt='+format+
'; domain='+CL.url.baseDomain+'; expires='+
