...
catch(e){}};}}},when:{localStorageAvailable:$.Deferred()}});(function(safe){CL.extend({'JSON':{parse:safe(JSON.parse,JSON),stringify:safe(JSON.stringify,JSON)},'localStorage':{getItem:safe(localStorage.getItem,localStorage),setItem:safe(localStorage.setItem,localStorage),removeItem:safe(localStorage.removeItem,localStorage)},'jls':{getItem:safe(function(key){return JSON.parse(localStorage.getItem(key));}),setItem:safe(function(key,value){return localStorage.setItem(key,JSON.stringify(value));})}});})(CL.util.safe);if(CL.url.baseDomain){document.domain=CL.url.baseDomain;$('body').append('<iframe style="display: none;" src="//www.'+CL.url.baseDomain+'/about/localstorage"></iframe>');}else{CL.when.localStorageAvailable.reject();}
var getCLFmt=function(){return(document.cookie.match('(^|; )cl_fmt=([^;]*)')||0)[2];};$(document).ready(function(){formats_autosize(pagetype);ie_tweaks();});function ie_tweaks(){if(CL.browser.ieVersion<=7){var $crumbs=$('.crumb');if($crumbs.length>1){$crumbs.length=$crumbs.length-1;$crumbs.append(' >');}}}
function formats_autosize(pagetype){if(pagemode==='mobile'){if(typeof getCLFmt()==='undefined'){firstTimeMobileFormat();}
...
function update_format_selector(){$('#fmtsel .fsel').each(function(){if($(this).attr('data-mode')===pagemode){$(this).contents().wrap('<b>');}else{$(this).contents().wrap('<a href="#">');}}).on('click',function(e){e.preventDefault();var target_mode=$(this).attr('data-mode');if(target_mode!==pagemode){changeFormat(target_mode);}});}
...
return isoDate;})
.timeago()
.on('click',function(){var $this=$(this);var temp=$this.text();$this.text($this.attr('title'));$this.attr('title',temp);});var $thumbs=$('#thumbs a');var $figure=$('figure.iw');var sliderHtml='';if($thumbs.length>1){sliderHtml+='<div class="slidernav"><button class="back" disabled="disabled">&lt;</button>'+
